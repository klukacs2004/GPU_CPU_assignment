...
public:
    ...
    DiaGenParameters(T x_min_, T x_max_, T r_min_, T r_max_, int resolution_x, int resolution_y) : x_axis(resolution_x),
                                                                                                   y_axis(resolution_y),
                                                                                                   x_min(x_min_),
                                                                                                   x_max(x_max_),
                                                                                                   r_min(r_min_),
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (arg == "--xmin")
            ...
            x_max = std::stof(argv[++i]);
        else if (arg == "--rmin")
            r_min = std::stof(argv[++i]);
        else if (arg == "--rmax")
            r_max = std::stof(argv[++i]);
        else if (arg == "--nx")
            nx = std::stoi(argv[++i]);
        else if (arg == "--ny")
            ny = std::stoi(argv[++i]);
